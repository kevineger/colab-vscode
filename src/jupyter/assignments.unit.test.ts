  ...
  describe("unassignServer", () => {
    ...
    describe("when a server created in VS Code exists", () => {
      ...
      beforeEach(async () => {
        ...
        colabClientStub.jupyter.withArgs(defaultServer).returns(jupyterStub);
      });
      ...
      it("deletes sessions", async () => {
        ...
        sinon.assert.calledWith(jupyterStub.sessions.delete, { session: session1.id });
        sinon.assert.calledWith(jupyterStub.sessions.delete, { session: session2.id });
      });
